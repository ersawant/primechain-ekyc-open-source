    ...
    download_document: (req, res, next) => {
        try {
            ...
            if (publisher_address === req.user.user_address) {
                formModel.getEncryptionKeyByDocumentTxid(document_txid, (err, result) => {
                    if (err) { return next(err); }

                    if (result) {
                        // Querying the data from the blockchain
                        dataStorage.getStreamItem(config["STREAMS"]["KYC_RECORD_STREAM"], document_txid, (err, data_info) => {
                            if (err) { return next(err); }
                        ...
                        // flash is used to display message in the view page.
                        req.flash("error_msg", "Unable to fetch key");
                        res.redirect(common_utility.refererPathExtractor(req));
                    }
                });
            }
